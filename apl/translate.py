...
from urllib.request import HTTPRedirectHandler, HTTPDefaultErrorHandler, HTTPError, build_opener, quote

from django.conf import settings

### Hard-coded variables ###
api = settings.GOOGLE_TRANSLATION_KEY
api = 'AIzaSyCrt10gD_9slbq3cZ7YkcL4vmoFMD116to'
...
class GoogleTranslator(object):
    ...
    def _build_uri(self, extra_url, params):
        ...
        url = "%s?%s" % (urljoin(self.base_url, extra_url), params)
    ...
    def _sanitize_query(self, query):
        if isinstance(query, (list,tuple)):
            query = zip('q' * len(query), map(quote,query))
        else:
            query = [('q',quote(query))]
        return query
